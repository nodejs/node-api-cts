...
static napi_value TestBoolValuedPropApi(
    napi_env env,
...
static napi_value SetElement(napi_env env, napi_callback_info info) {
  ...
  napi_set_element(env, object, 0, NULL);
  add_last_status(env, "valueIsNull", return_value);
...
static napi_value GetElement(napi_env env, napi_callback_info info) {
  ...
  napi_get_element(env, NULL, 0, &prop);
  add_last_status(env, "objectIsNull", return_value);

  napi_get_element(env, object, 0, NULL);
  add_last_status(env, "valueIsNull", return_value);

  return return_value;
}

static napi_value TestBoolValuedElementApi(
    napi_env env, napi_status (*api)(napi_env, napi_value, uint32_t, bool*)) {
  napi_value return_value, object;
  bool result;
...
static napi_value DefineProperties(napi_env env, napi_callback_info info) {
  ...
  napi_property_descriptor desc = {"prop",
                                   NULL,
                                   DefineProperties,
                                   NULL,
                                   NULL,
  ...
  // Upstream quirk carried over deliberately: the two cases below mutate
  // `desc` but still pass a NULL descriptor list, so they re-test
  // descriptorListIsNull rather than the field they are named after. Passing
  // `&desc` instead segfaults -- napi_define_properties dereferences a
  // descriptor that names no property and carries no value, getter, setter or
  // method. Reported upstream; the names are kept so the expectations match
  // nodejs/node.
  desc.utf8name = NULL;
  napi_define_properties(env, object, 1, NULL);
...
void init_test_null(napi_env env, napi_value exports) {
  ...
  NODE_API_CALL_RETURN_VOID(
      env, napi_define_properties(env, exports, 1, &test_null_set));
}
