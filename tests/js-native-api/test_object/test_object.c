...
static napi_value Get(napi_env env, napi_callback_info info) {
  ...
  NODE_API_ASSERT(env,
                  ...
                  "Wrong type of arguments. Expects an object as first "
                  "argument.");
  ...
  NODE_API_ASSERT(env,
                  ...
                  "Wrong type of arguments. Expects a string or symbol as "
                  "second.");

  napi_value output;
  NODE_API_CALL(env, napi_get_property(env, args[0], args[1], &output));
...
static napi_value GetNamed(napi_env env, napi_callback_info info) {
  ...
  napi_value output;
  NODE_API_CALL(env, napi_get_named_property(env, args[0], key, &output));
...
// Returns the property names selected by the given filter/conversion, so the
// test can compare each napi_get_all_property_names mode against JS.
static napi_value GetAllNames(napi_env env,
...
static napi_value GetSymbolNames(napi_env env, napi_callback_info info) {
  return GetAllNames(
      env, info, napi_key_include_prototypes, napi_key_skip_strings);
}

static napi_value GetEnumerableWritableNames(napi_env env,
                                             napi_callback_info info) {
  return GetAllNames(env,
                     info,
...
static napi_value GetOwnWritableNames(napi_env env, napi_callback_info info) {
  return GetAllNames(env, info, napi_key_own_only, napi_key_writable);
}
...
static napi_value GetOwnConfigurableNames(napi_env env,
                                          napi_callback_info info) {
  return GetAllNames(env, info, napi_key_own_only, napi_key_configurable);
}
...
static napi_value HasOwn(napi_env env, napi_callback_info info) {
  ...
  // The key is deliberately not type-checked here: the test asserts that
...
static napi_value TestFreeze(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(env, napi_object_freeze(env, args[0]));
...
static napi_value TestSeal(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(env, napi_object_seal(env, args[0]));

  return args[0];
}

// Type tags are basically 128-bit UUIDs. The zero tag and the half-zero tags
// are included so an implementation cannot pass by treating "empty" as a
// wildcard.
...
// Tags `instance`, then wipes the local copy of the tag. A tagging
// implementation that stored a pointer rather than the 128-bit value would be
// left pointing at cleared stack memory, so CheckTypeTag would fail.
static napi_status tag_and_clear(napi_env env,
                                 napi_value instance,
                                 uint32_t type_index) {
  napi_type_tag tag = type_tags[type_index];
  NODE_API_CHECK_STATUS(napi_type_tag_object(env, instance, &tag));
...
static napi_value TypeTaggedInstance(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(env, tag_and_clear(env, instance, type_index));
