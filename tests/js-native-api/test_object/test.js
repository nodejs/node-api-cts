const test_object = loadAddon('test_object');
...
{
  // napi_get_property walks the prototype chain.
  function MyObject() {
    this.foo = 42;
    this.bar = 43;
  }
...
{
  // napi_has_own_property fails if the key is not a name.
...
{
  // napi_has_own_property does not walk the prototype chain.
...
{
  // Inflate increases every own enumerable property by 1.
  const cube = {
    x: 10,
    y: 10,
    z: 10,
...
{
  // Wrapping does not break an object's prototype chain.
  const wrapper = {};
...
{
  // The pointer can still be unwrapped after inserting a link in the
  // prototype chain.
  const wrapper = {};
  const protoA = { protoA: true };
  Object.setPrototypeOf(wrapper, protoA);
...
{
  // Objects can be type-tagged and type-tag-checked.
  ...
  // Type tag indices greater than the largest available index are rejected.
  ...
  // Type tags are correctly accepted.
  ...
  // Wrongly tagged objects are rejected.
...
{
  // Normal and nonexistent properties can be deleted.
  const sym = Symbol();
...
{
  // Non-configurable properties are not deleted.
  const obj = {};
...
{
  // Prototype properties are not deleted.
  function Foo() {
    this.foo = 'bar';
  }
...
{
  // napi_get_property_names gets the right set of property names: includes
  // prototypes, only enumerable properties, skips symbols, and includes
  // indices converted to strings.
  const object = {
    __proto__: {
      inherited: 1,
    },
...
// Passing NULL to napi_set_property reports the right error.
...
// Passing NULL to napi_has_property reports the right error.
...
// Passing NULL to napi_get_property reports the right error.
...

{
  ...
  // Test files are modules and therefore strict, so these violations throw
  // rather than fail silently. The message text is engine-specific, so only
  // the error type is asserted.
  assert.throws(() => {
    obj.w = 'd';
  }, TypeError);
  ...
  // Sealing still allows updating existing properties.
  obj.x = 'd';
  assert.strictEqual(obj.x, 'd');
}
